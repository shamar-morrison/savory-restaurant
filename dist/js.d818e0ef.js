...
},{"_css_loader":"node_modules/parcel-bundler/src/builtins/css-loader.js"}],"node_modules/fslightbox/index.js":[function(require,module,exports) {
var define;
...
var _index = _interopRequireDefault(require("../../node_modules/fslightbox/index"));
...
var quoteIcon = document.querySelector('.quote-icon');
var heroSliderNavBullets = document.querySelector('.hero__slider-nav--dots');
var pauseHeroSliderBtn = document.querySelector('.btn-pause-slider');
var numbersSec = document.querySelector('#numbers');
var mobileAppSec = document.querySelector('#mobile-app');
var powerDealSec = document.querySelector('#power-deal');
var mobileAppBtns = document.querySelector('.section__no-bg.mobile-app .dual-btns');
var mobileAppTitle = document.querySelector('.mobile-app--title');
var powerDealBtns = document.querySelector('.section__no-bg.bogof .dual-btns');
var powerDealTitle = document.querySelector('.power-deal--title');
var isHeroSliderPaused = false;
...
window.onload = function () {
  ...
  desertsFoods.style.transform = 'translateY(-100%) scale(0.95)'; // init hero slider

  setInterval(function () {
    if (isHeroSliderPaused) return;
    animHeroSlideText();
    animBgImages();
  }, 5000); // init review slider quote icon anim
...
var hideActiveList = function hideActiveList(foodList) {
  ...
  foodList.classList.add('hidden-food'); // if foodList !== 'main dishes' => translateY(-100%)
  // to keep original position and prevent the food list
  // from breaking position on page

  foodList.style.transform = "".concat(foodList !== mainFoods ? 'translateY(-100%)' : '', " scale(.95)");
}; // make food list active


var makeActiveList = function makeActiveList(foodList) {
  foodList.classList.remove('hidden-food');
  foodList.classList.add('active-food');
  foodList.style.transform = "".concat(foodList !== mainFoods ? 'translateY(-100%)' : '', " scale(1)");
};

var updateBtn = function updateBtn(btn) {
...
var updateMenu = function updateMenu(foodList, foodBtn) {
  if (foodList.classList.contains('active-food')) return; // hide previously active food list

  var activeFood = document.querySelector('.active-food');
  hideActiveList(activeFood); // make active food list

  makeActiveList(foodList); // update selected btn

  updateBtn(foodBtn);
}; // menu animation


menuBtns.addEventListener('click', function (event) {
...
mobileToggle.addEventListener('click', toggleMobileMenu); // close mobile menu on link click
...
var imgThree = document.querySelector('.img-3'); // make BG image active

var removeActiveImg = function removeActiveImg(htmlEl) {
  htmlEl.classList.remove('active-bg-img');
  htmlEl.classList.add('not-active-bg-img');
}; // make BG image not active


var addActiveImg = function addActiveImg(htmlEl) {
...
var heroSlideThree = document.querySelector('.slide-three');

var updateSliderBullet = function updateSliderBullet(slide) {
  document.querySelector('.hero-nav-dot-active').classList.remove('hero-nav-dot-active');
  document.querySelector("[data-slide-to=".concat(slide, "]")).classList.add('hero-nav-dot-active');
}; // pause/resume hero slider


var pauseHeroSlider = function pauseHeroSlider() {
  isHeroSliderPaused = !isHeroSliderPaused; // update icon
...
var makeTextSlideNotActive = function makeTextSlideNotActive(slide) {
  var isFirstSlide = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  slide.classList.remove("".concat(isFirstSlide ? 'slide-active' : 'slide-active-pos'));
  slide.classList.add("".concat(isFirstSlide ? 'slide-not-active' : 'slide-not-active-pos'));
};

var makeTextSlideActive = function makeTextSlideActive(slide) {
  var isFirstSlide = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
  slide.classList.add("".concat(isFirstSlide ? 'slide-active' : 'slide-active-pos'));
  slide.classList.remove("".concat(isFirstSlide ? 'slide-not-active' : 'slide-not-active-pos'));
};
...
heroSliderNavBullets.addEventListener('click', function (event) {
  if (!event.target.dataset.slideTo) return;
  var slide = event.target.dataset.slideTo;

  switch (slide) {
    case 'slide-one':
      {
        // if slide-one is current slide, don't switch slides
        if (heroSlideOne.classList.contains('slide-active')) return; // make hero slide-one active

        makeTextSlideActive(heroSlideOne, true);
        makeTextSlideNotActive(document.querySelector('.slide-active-pos')); // make img-1 active
    ...
    case 'slide-two':
      {
        if (heroSlideTwo.classList.contains('slide-active-pos')) return; // make hero slide-2 active

        makeTextSlideActive(heroSlideTwo); // if slide-one was previous slide
...
var counters = document.querySelectorAll('.numbers__block--num'); // the speed at which to animate. The lower the number, the faster the anim

var countSpeed = 99;

var animNumbers = function animNumbers(event) {
  if (!event[0].isIntersecting) return;
  counters.forEach(function (counter, i) {
    var updateCount = function updateCount() {
      var target = +counter.dataset.count; // number to increment to

      var count = +counter.innerText;
      var inc = target / countSpeed;

      if (count < target) {
        counter.innerHTML = "".concat(Math.trunc(count + inc));
        setTimeout(updateCount, 1);
      } else {
        counter.innerHTML = "".concat(target, " ").concat(i === 2 ? '<span class="icon small">%</span>' : '<span class="icon">+</span>');
      }
    };
...
var numbersObs = new IntersectionObserver(animNumbers, {
  threshold: 1
});
...
var animateOut = function animateOut(el) {
  var distance = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : '350%';
  el.style.transform = "translateY(".concat(distance, ") scale(.7)");
  el.style.opacity = '0';
};
...
var animSec = function animSec(event, elOne, elTwo) {
